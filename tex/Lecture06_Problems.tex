...
\begin{frame}
\chtitle{方均根速率$\ge$平均速率(第二种证明方法及推广，仅供娱乐)}
\bch
{\blue 教材思考题2-9}
...
\begin{frame}
\chtitle{方均根速率$\ge$平均速率 （第三种证明方法及推广，仅供娱乐）}
\bch
{\blue 教材思考题2-9}
...
\overline{|x|^n} &=& -\sqrt{\frac{2}{\pi}}\int_0^\infty x^{n-1}d\left(e^{-x^2/2}\right) \newl
 &=& \left.-\sqrt{\frac{2}{\pi}}x^{n-1}e^{-x^2/2}\right\vert_0^\infty+\sqrt{\frac{2}{\pi}}\int_0^\infty (n-1)x^{n-2} e^{-x^2/2}dx \newl
...
\begin{frame}
\chtitle{Maxwell分布例5 （尾区间积分，仅供娱乐）}
\bch
{\blue 教材习题2-15}
...
\begin{frame}
\chtitle{Maxwell分布例6 （大区间积分，仅供娱乐）}
\bch
{\blue 教材习题 2-14}
...
\begin{frame}
\chtitle{Maxwell分布例7 （大区间积分和迭代法，仅供娱乐）}
\bch
{\blue 教材思考题2-10}
...
\begin{frame}
\chtitle{Maxwell分布例9 （绝热箱泻流，难度稍大的内容）}
\bch
...
当然，如果你不大放心，可以直接计算：
\be
\overline{E_{\rm grav}} = \frac{\int_0^\infty e^{-\frac{mgz}{kT}} mgz dz}{\int_0^\infty e^{-\frac{mgz}{kT}}dz} =kT
\ee
