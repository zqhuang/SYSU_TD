...
括号内的积分可以看成标准正态分布变量的平方平均（注意这仅仅是在数学上相等，$u$本身并不服从正态分布），故按递归公式等于$1$。所以$\bar{u} = \sqrt{\pi/2}$，即
...
\eea
上式可以看成标准正态分布变量$x$的 $|x|$的平均（注意这仅仅是在数学上相等，$u$本身并不服从正态分布），故等于 $\sqrt{\frac{2}{\pi}}$。再换回到普通单位制：
