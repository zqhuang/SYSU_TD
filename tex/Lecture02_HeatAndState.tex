...
\begin{frame}
\chtitle{乘着你们已凌乱，必须补刀}
\bch
\addfig{1.2}{budao.png}
...
\begin{frame}
\chtitle{题外话：燃鹅，这个体积修正的解释并不靠谱…（本页仅供娱乐）}
\bch
{\small 黄俊康同学对这个解释发起了挑战:按照推导动理压强的方法，$n$应该是单位体积内的平均粒子数，不是单位未占据空间内的平均粒子数…

\skipline

逼得我不得不写了个程序模拟刚性球分子气体：

\bmini{0.5}
\addfig{2}{vcorr.pdf}
\emini
\bmini{0.45}
体积固定为1。X和{\blue $\bullet$}均为模拟数据，其中{\blue $\bullet$}模拟采用的总分子数是X的两倍。红色为拟合直线。

\skipline

可见刚性球模型的体积修正其实和分子半径(而不是分子体积)成线性关系。而且，跟摩尔数没啥关系…
\emini
