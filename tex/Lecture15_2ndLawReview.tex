...
\begin{frame}
  \chtitle{第一种套路: 直接告诉$Q$和$T$}
  \bch
  \tbox{$$\Delta S = \frac{Q}{T}$$}
  
  \bigjianxiao 最喜欢这种侮辱智商的代公式题
...
\begin{frame}
  \chtitle{第二种套路: 不等温热库间传热}
  \bch
  温度为$T_1$的热库1向温度为$T_2$的热库2传热$Q$ ($T_1>T_2, Q>0$)。这可以看成热库1吸收$-Q$的热量，热库2吸收$Q$的热量，总熵变为：

  \tbox{$$\Delta S = -\frac{Q}{T_1}+\frac{Q}{T_2}$$}
  
  \bigbye 我觉得代两次公式已经是你的智商上限
...
\begin{frame}
  \chtitle{第五种套路：利用一般$pVT$系统恒等式}
  \bch
  \tbox{$$\dbar Q = C_VdT + \pheat dV $$}
  \tbox{$$\dbar Q = C_pdT - \vheat dp $$}  
  \addfig{1.2}{shaodiantaolu.jpg}    
  \ech
\end{frame}
