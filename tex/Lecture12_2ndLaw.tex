...
\bch
如果一个系统虽然处于非平衡态，但很接近温度为$T$的平衡态（即在不受外界影响的情况下，它会很快达到温度为$T$的平衡态），我们有理由近似认为它的“温度”近似为$T$。实际上，日常生活中很少有完美地处于平衡态的物体，我们所说的温度都是指这个近似意义下的温度。
\ech
\end{frame}
...
$\dbar Q = (T+\delta T) dS_{\rm eq}$似乎更为妥当，其中的$\delta T$并无明确的数值，只是作为系统偏离平衡态的一种定性描述。但是，当你写一个一阶微分等式或者不等式时，任何二阶小量都要舍去。所以无论你是否加上一个$\delta T$，对结果无影响。}
