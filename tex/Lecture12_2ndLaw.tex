...
\item{理想气体循环效率的计算: 一般在$T$-$S$图上计算比较方便。有时要用到理想气体熵变：
  {\blue $$ dS = C_V d\ln T + \nu R d\ln V$$}
  把熵理解为微观态数目的对数的话，第一项代表动量空间的大小改变带来微观态数目的变化，第二项代表位置空间大小的改变带来微观态数目的变化。(还记得微观态数目正比于相空间体积吗？)
}
\eitem
\ech
\end{frame}
...
在这个例子中，我们可以把热传导棒划分成$N$段，每相邻两段的温度差$\Delta T$为$1/N$的量级，相邻两端之间传递的热量$Q$确实固定的有限大小。于是$N\Delta T Q$并不趋向于零。故是不可逆过程。
\ech
\end{frame}
