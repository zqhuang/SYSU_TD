...
$$\sum_{\upsilon_x>0}(2p_x)\left[n(\vecv) dS (\upsilon_x dt)  \right]$$
其中$n(\vecv)$是速度为$\vecv$ (满足$\upsilon_x>0$)的分子数密度；动量的x分量$p_x= m\upsilon_x$(每次弹性反射都传递动量$2p_x$)。
\emini

因为统计上$\upsilon_x>0$和$\upsilon_x<0$的粒子数密度相等，故总分子数密度$n = \sum_{\upsilon_x>0} 2n(\vecv)$，所以压强为
\bmini{0.66}
\bea
2\sum_{\upsilon_x}p_x\upsilon_x n(\vecv) &=& n \frac{\sum_{\upsilon_x>0}p_x\upsilon_x n(\vecv)}{\sum_{\upsilon_x>0} n(\vecv)} \newl
&=& \left. n \overline{p_x \upsilon_x} \right\vert_{\upsilon_x>0} \newl
&=& n \overline{p_x \upsilon_x} \newl
&=& \frac{1}{3}n\left(\overline{p_x \upsilon_x} +  \overline{p_y \upsilon_y} + \overline{p_z \upsilon_z} \right) \newl
&=& \frac{1}{3}n\, \overline{\mathbf{p}\cdot\vecv}
\eea
\emini
\bmini{0.3}
\vspace{0.35in}
...
\bch
非相对论气体$\overline{\mathbf{p}\cdot\vecv} = m\overline{\upsilon^2} = 2\overline{\varepsilon} = 3 k T$。
...
\bch
{\scriptsize
在非相对论情形下，平动、转动、小振幅振动的能量都可以写成（广义）动量的二次型。假设微观粒子有多个独立的自由度，则我们仅需取其中某自由度考虑，其能量为$ c p^2$ ($c$为正常数），（广义）动量在$p$和$p+dp$之间的状态数正比于$dp$(现在是一维的情况就不需要乘以$4\pi p^2$了)，该自由度动量在$p$附近的概率正比于$e^{-\frac{cp^2}{kT}}$。所以该自由度平均能量为
\bea
\frac{\int_{-\infty}^{\infty} cp^2e^{-\frac{-cp^2}{kT}}dp }{\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp } &=& \frac{-\frac{kT}{2}\int_{-\infty}^{\infty} p d \left(e^{-\frac{-cp^2}{kT}}\right)  }{\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp }  \newl
&=& \frac{-\frac{kT}{2}\left[\left.p e^{-\frac{-cp^2}{kT}}\right\vert_{-\infty}^{\infty}-\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp\right]  }{\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp }  \newl
&=& \frac{\frac{kT}{2}\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp  }{\int_{-\infty}^{\infty} e^{-\frac{-cp^2}{kT}}dp }  \newl
&=& \frac{kT}{2}
\eea
}
\ech
...
\item[2]{第一章习题1-7}
\item[3]{把1 mol的理想气体保持恒温300K进行等温压缩，使得体积变为原来一半，要做多少功？}
\eitem
