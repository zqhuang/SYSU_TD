...
\begin{frame}
  \chtitle{套路1：初态两个独立量 + 末态一个独立量}
  \bch
  
  {\scriptsize (理想气体$\Delta U = 0$和$\Delta H=0$(节流)都等价于$T_2=T_1$。)}
  
  \addfig{0.8}{doushitaolu.jpg}
...
\begin{frame}
  ...
  \bch
  \bmini{0.6}
  \lfig{2.5}{PTdiagram.png}
  \emini
  \bmini{0.36}
  把$pVT$图的$V$自由度投影掉之后，相变曲面成为相变曲线。这时可以省略固定$V$的下标。写成
  \emini

  \tbox{  $$\frac{dp}{dT}= \frac{\Lambda^{\rm mol}}{T\left(V^{\rm mol}_\beta - V^{\rm mol}_\alpha\right)} $$  }
  \ech
\end{frame}
