...
由于$p, V, T$中只有两个是独立的，我们也可以把内能写成$U(p, T)$。那么实际气体的$\pfrac HpT$的符号一般是什么呢？
...
\bch
\tbox
{系统内能的增加$\Delta U$ = 从环境吸热$Q$ $-$  系统对环境做功$A'$}
...
\bch
\tbox
{系统内能的增加$\Delta U$ = 环境对系统做功$A$ $-$ 对环境放热$Q'$}
...
\bch
\tbox
{系统内能的增加$\Delta U$ = 从环境吸热$Q$ $+$ 环境对系统做功$A$}
\ech
\end{frame}
...
\bch
显然，因为只和分子平均距离有关，{\blue 全局势能和内压强只是体积的函数}。所以固定体积时，
...
\begin{frame}
\chtitle{非准静态过程的本质和热力学第二定律}
\bch
{\blue 当宏观过程发生得很快，物质和环境的分子碰撞来不及提供动理压强做功和改变局部内能，这样的过程就是非准静态过程。非准静态过程是不可逆的，也称为不可逆过程。}

现在考虑一个温度变化$\Delta T/T$很小的任意过程，设初态和末态均为平衡态，则
{\blue
$$ Q \le T \Delta S $$
当且仅当过程为准静态时，等号成立}。这就是{\blue 热力学第二定律}。

因孤立系统的$Q=0$，故{\blue $\Delta S\ge 0$}，这就是{\blue 孤立系统的熵不减原理}。
\ech
\end{frame}
...
\chtitle{热机效率和卡诺定理}
\bch
考虑一个从温度为$T_1$的高温热源吸热$Q_1$，向温度为$T_2$的低温热源放热$Q_2'$，做功$A = Q_1-Q_2'$，并回到初始状态的热机循环过程。
根据热二律：
$$\frac{Q_1}{T_1} - \frac{Q_2'}{T_2} \le \Delta S_1 + \Delta S_2 = 0 $$
故$$\frac{Q_2'}{Q_1} \ge \frac{T_2}{T_1}$$
即热机效率
{\blue $$\eta \equiv \frac{A}{Q_1}  = 1 - \frac{Q_2'}{Q_1} \le 1 - \frac{T_2}{T_1}$$
等号当且仅当热机经历的过程均为可逆过程（即热机为可逆热机）时成立}
这就是{\blue 卡诺定理}
\ech
\end{frame}
...
\begin{frame}
\bch
\bigwulian {\Large 一不小心没刹住车把三四章都讲完了}
\ech
\end{frame}
