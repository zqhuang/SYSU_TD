...
{\scriptsize
\bitem
\item[1]{转换到以$x$轴为南北极的球坐标系并对$\theta$ (或$\mu\equiv \cos\theta$), $\varphi$积分，得到$F(u) \propto \frac{1}{24} u^4 e^{-u}$。}
...
\item[5]{$$u_{n, x, \rm leak} = \frac{\int_0^\infty u^5du \int_0^1\mu^3 d\mu \int_0^{2\pi} d\varphi}{\int_0^\infty u^4du \int_{-1}^1\mu^2 d\mu \int_0^{2\pi} d\varphi} = \frac{15}{8} $$}
\item[6]{改取$z$轴为南北极方向，
$$u_{n, z, \rm leak} = \frac{\int_0^\infty u^5du \int_0^{\pi/2} \cos\theta \sin^3\theta d\theta \int_0^{2\pi} \cos^2\varphi d\varphi}{\int_0^\infty u^4du \int_{0}^{\pi} \sin^3\theta d\theta \int_0^{2\pi} \cos^2\varphi d\varphi} = \frac{15}{16}$$}
\eitem
}
\ech
\end{frame}
